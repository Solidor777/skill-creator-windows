...
def _kill_proc(process) -> None:
    """Kill a subprocess if still running (used by the timeout watchdog)."""
    try:
...
def run_single_query(
    ...
    try:
        ...
        # Windows-compatible read: select() only works on sockets on Windows,
        # not on subprocess pipes, so read the stream line-by-line (blocking)
        # and use a watchdog timer to kill the process if it exceeds the timeout.
